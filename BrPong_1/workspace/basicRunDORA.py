...
class runDORA(object):
    def __init__(self, memory, parameters):
        ...
        self.count_by_RBs = parameters['count_by_RBs'] # initialize to None.
        self.local_inhibitor_fired = False # initialize to False.
        self.max_norm = parameters['max_norm']
        self.one_to_one = parameters['one_to_one']
        self.len_regularisation = parameters['len_regularisation']
        self.parent_bias = parameters['parent_bias']
        self.order_activation_bias = parameters['order_activation_bias']
        self.activation_storage_path = parameters['activation_storage_path']
    ...
    def create_firing_order(self):
        if self.count_by_RBs:
            ...
            self.firingOrder = []
            for unit_P in self.memory.driver.Ps:
                if '.' not in unit_P.name: # only include original props
                    unit_POs = get_POs(unit_P)
                    for unit_PO in unit_POs:
                        self.firingOrder.append(unit_PO)
    ...
    # 4) Enter the phase set. A phase set is each RB firing at least once (i.e., all RBs in firingOrder firing). 
    ...
    def do_map(self):
        ...
        phase_sets = 3
        # if there are multiple relations in the driver (i.e., the number of P units is 2 or more), then switch to LISA mode, 
        # and set ignore_object_semantics to True.
        changed_mode = False
        ...
        for phase_set in range(phase_sets):
            ...
            if self.count_by_RBs:
                for currentRB in self.firingOrder:
                    ...
                    # 4.1-4.2) Fire the current RB in the firingOrder. Update the network in discrete time-steps until
                    # the globalInhibitor fires (i.e., the current active RB is inhibited by its inhibitor).
                    ...
                    while self.memory.globalInhibitor.act == 0:
                        ...
                        self.memory = update_mappingHyps(self.memory, self.order_activation_bias)
                ...
                for currentPO in self.firingOrder:
                    ...
                    while self.memory.localInhibitor.act == 0:
                        # 4.3.1-4.3.10) update network activations.
                        currentPO.act = 1.0
                        self.time_step_activations(phase_set, False, False)#self.ignore_object_semantics, self.ignore_memory_semantics) #KK
                        # 4.3.11) Update mapping hypotheses.
                        ...
                        if self.doGUI:
                            self.time_step_doGUI(phase_set_iterator)
                    # PO firing is OVER.
                    self.post_count_by_operations(kk_style=False) # to make sure RB reset does not happend
    ...
    def do_retrieval(self,store_activations=False,store_iteration=None,array_PO=None,array_RB=None,array_P=None):
        # do initialize network operations (steps 1-3 above). 
        # do initialize network operations (steps 1-3 above).
        self.do_1_to_3(mapping=False) # added by KK - how is it managing to work without?!
        phase_sets = 1
        for phase_set in range(phase_sets):
            ...
            if self.count_by_RBs:
                ...
                for currentPO in self.firingOrder:
                    ...
                    while self.memory.localInhibitor.act == 0:
                        ...
                        self.time_step_activations(phase_set, self.ignore_object_semantics, self.ignore_memory_semantics)
                        # 4.3.12) Run retrieval routines.
                        self.memory = retrieval_routine(self.memory, self.asDORA, self.gamma, self.delta, self.HebbBias,
                                                        self.lateral_input_level, self.bias_retrieval_analogs)
                        # fire the local_inhibitor if necessary.
                        self.time_step_fire_local_inhibitor()
                        # update GUI.
                        phase_set_iterator += 1
                        if self.doGUI:
                            self.time_step_doGUI(phase_set_iterator)
                        # PO firing is OVER.
                        if store_activations: # KK
                            array_PO, array_RB, array_P = get_all_activations(self.memory,array_PO,array_RB,array_P) # add activations
                    self.post_count_by_operations()
            ...
            if store_activations:
                return array_PO, array_RB, array_P
                #self.write_all_activations(store_iteration,array_PO,array_RB,array_P)

    def write_all_activations(self,iter_number,array_PO,array_RB,array_P):
        # write to file - activation_PO_iter
        np.savetxt(self.activation_storage_path+'POs/PO_'+str(iter_number)+'.csv', array_PO)
        np.savetxt(self.activation_storage_path + 'RBs/RB_' + str(iter_number)+'.csv', array_RB)
        np.savetxt(self.activation_storage_path + 'Ps/P_' + str(iter_number)+'.csv', array_P)
    ...
    def do_compression(self):
        ...
        if len(to_compress_objects) > 0:
            ...
            for obj in to_compress_objects:
                # the object controls firing. Fire the object, nd learn a copy of object and RB in emerging recipient. 
                # Then, fire each pred and learn compressed version in emerging recipient.
                ...
                # for each compressed object, there will have a new RB in newSet that conjoins the object to the 
                # compressed pred. Initialise made_RB to None.
                made_RB = None
    ...
    def time_step_activations(self, phase_set, ignore_object_semantics=False, ignore_memory_semantics=False):
        ...
        ## self.asDORA is false for our case 
    ...
    def post_count_by_operations(self, kk_style=False):
    ...
    def post_phase_set_operations(self, retrieval_license, map_license, inferred_new_P=False):
        # if you were doing retrieval (i.e., if retrieval_license is True), then use the Luce choice axiom here to
        # retrieve items from memorySet into the recipient.
        ...
        if map_license:
            ...
            self.memory = update_mappingConnections(self.memory, self.eta, self.one_to_one, self.max_norm, self.len_regularisation,
                                                    self.parent_bias) # CHANGED BY KK
...
def make_AM(memory):
    # for each token, if it is in an AM set (driver, recipient), then make sure all sub-tokens are also in that set.
    # Make sure that all tokens from the same analog are in the same set or in memory (i.e., tokens from the same analog
    # CANNOT be in different AM sets). run findDriverRecipient().
    # for each token unit, make sure all subtokens are in the same set. Also, make sure that if a token is to enter
    # recipient, that it checks to make sure none of it's subtokens are in the driver, and, if they are, that it remains in memory.
...
def findDriverRecipient(memory, full_pick=False):
    ...
    for myP in memory.Ps:
        ...
        ##### CHANGED CODE HERE : in the previous step in DORA mode, the P's are randomly assigned to driver.
        ##### But the ones that are left out are not assigned to recipient, which is what we do here (need to check if
        ##### this is expected behavior)
        elif myP.set == 'memory' and full_pick:
            memory.recipient.Ps.append(myP)
            # reset the .copy_for_DR field back to False.
            myP.copy_for_DR = False
            # now add the analog to recipient.analogs if it is not already there.
    ...
    for myRB in memory.RBs:
        ...
        ##### But the ones that are left out are not assigned to recipient, which is what we do here (need to check if
        ##### this is expected behavior)
        elif myRB.set == 'memory' and full_pick:
            memory.recipient.RBs.append(myRB)
            # reset the .copy_for_DR field back to False.
            myRB.copy_for_DR = False
            # now add the analog to recipient.analogs if it is not already there.
    ...
    for myPO in memory.POs:
        ...
        ##### But the ones that are left out are not assigned to recipient, which is what we do here (need to check if
        ##### this is expected behavior)
        elif myPO.set == 'memory' and full_pick:
            memory.recipient.POs.append(myPO)
            # now add the analog to recipient.analogs if it is not already there.
            if myPO.myanalog not in memory.recipient.analogs:
                memory.recipient.analogs.append(myPO.myanalog)
...
def makeFiringOrder(memory, rule):
    ...
    if rule == 'in_order':
        # firing order is the same as arrangement of Ps in the driver
        if len(memory.driver.Ps) > 0:
        ...
        if len(memory.driver.Groups) > 0:
            ...
            if len(memory.driver.RBs) > 0:
                ...
                for myPO in memory.driver.POs:
                    firingOrder.append(myPO)
                random.shuffle(firingOrder)
    if rule == 'totally_random': # use a totally random firing order.
...
def update_memory_inputs(memory, asDORA, lateral_input_level):
    # for all units not in driver, recipient, or newSet (i.e., units with set != driver, recipient, or newSet), update input. 
    # Units in memory update as units in recipient.
    # set phase_set to 2.
    phase_set = 2
    for Group in memory.Groups:
        if Group.set == 'memory':
...
def resetMappingUnits(memory):
    ...
    # by KK - don't reset mappingConnections, reset only hyps
...
def update_mappingHyps(memory, order_activation_bias=0.0, recursive=2):
    #pdb.set_trace()
    if recursive == 0:
        for hypothesis in memory.mappingHypotheses:
            # do this for all levels of abstraction, with only one level - this is recursive
            if hypothesis.driverToken.my_type == 'P':
                correlation_vals = get_correlations(l1=flatten(get_activations_of_children(hypothesis.driverToken)),
                                                    l2=flatten(get_activations_of_children(hypothesis.recipientToken)))
                if correlation_vals != correlation_vals:
                    correlation_vals = 0.0 # don't allow nans to get through
            else:
                correlation_vals = 0.0
            hypothesis.update_hyp(memory, order_activation_bias, correlation_vals)
    elif recursive == 1:
        for hypothesis in memory.mappingHypotheses:
            if hypothesis.driverToken.my_type == 'P' or hypothesis.driverToken.my_type == 'RB':
                correlation_vals = get_correlations(l1=flatten(get_immediate_acts(hypothesis.driverToken)),
                                                      l2=flatten(get_immediate_acts(hypothesis.recipientToken)))
            else:
                correlation_vals = 0.0
            hypothesis.update_hyp(memory,order_activation_bias,correlation_vals)
    elif recursive == 2:
        for hypothesis in memory.mappingHypotheses:
            correlation_vals = 0.0
            if hypothesis.driverToken.unit_level == hypothesis.recipientToken.unit_level:
                if hypothesis.driverToken.my_type == 'P' or hypothesis.driverToken.my_type == 'RB':
                    correlation_vals = get_correlations(l1=flatten(get_immediate_acts(hypothesis.driverToken)),
                                                          l2=flatten(get_immediate_acts(hypothesis.recipientToken)))
                    if correlation_vals != correlation_vals: # handle nans
                        correlation_vals = 0.0
            hypothesis.update_hyp(memory,order_activation_bias,correlation_vals)
    return memory
...
def get_RB_length(rb_unit):
    len_rb_unit = 0
    if len(rb_unit.myChildP) > 0:
        for childP in rb_unit.myChildP:
            len_rb_unit += get_P_length(childP)
    else:
        if rb_unit.myPred[0].name != 'det': # this means the predicate is only a placeholder, don't consider
            len_rb_unit += 1 # Since only binary branching considered, only one predicate, and one object
        if rb_unit.myObj[0].name != 'det': # empty object do not consider
            len_rb_unit += 1
    return len_rb_unit

def get_P_length(p_unit):
    len_p_unit = 0
    for rb_unit in p_unit.myRBs:
        len_p_unit += get_RB_length(rb_unit)
    return len_p_unit

# update mapping connections.
def update_mappingConnections(memory, eta, one_to_one=True, max_norm=True, len_regularisation=1.0, parent_bias=0.1):
    # first step: divisively normalize all mapping hypotheses. For each mappng hypothesis divide it by the highest
    # hypothesis of either unit involved in that hypothesis. For example, for the mapping hypothesis between P[i] and
    # P[j] divide by max(max(hypothesis involving P[i]), max(hypothesis involving P[j])).
    #############################
    # BROKEN?
    #pdb.set_trace()
    #############################
    # ADDED BY KK - Try regular L1 normalisation instead of max_norm - the idea is to distribute mappings instead
    # of keeping it clustered
    # ADDING the equal length constraint for Ps only - necessary but not sufficient from the model
    # define length(proposition) = total number of downstream tokens connected to it
    def print_modes(hyp):
        print('driver : ', hyp.driverToken.name, ', mode : ',hyp.driverToken.mode)
    for hypothesis in memory.mappingHypotheses:
        if hypothesis.driverToken.my_type == 'P':
            driver_p_length = get_P_length(hypothesis.driverToken)
            recipient_p_length = get_P_length(hypothesis.recipientToken)
            if driver_p_length == recipient_p_length:
                hypothesis.length_bias = len_regularisation * (1.0/driver_p_length)
            if len(hypothesis.driverToken.myParentRBs) != len(hypothesis.recipientToken.myParentRBs):
                hypothesis.parent_bias = parent_bias
        hypothesis.hypothesis += hypothesis.length_bias
        hypothesis.hypothesis = hypothesis.hypothesis - hypothesis.parent_bias
    ...
    ## ADDED BY KK
    else: # instead of applying max norm, divide by sum of all hypotheses > 0
        for hypothesis in memory.mappingHypotheses:
            all_positive_hyp = 0 # maintain a sum of all hyps > 0
            for hyp in hypothesis.driverToken.mappingHypotheses: # only for driver, not recipient
                if hyp.hypothesis > 0:
                    all_positive_hyp += hyp.hypothesis
            # now divide every hypothesis by sum of all_positive_hyp
            if all_positive_hyp > 0:
                hypothesis.hypothesis /= all_positive_hyp


    # second step: subractively normalize each hypothesis.
    # For each hypothesis, look at all hypotheses of the driver and recipient unit sharing the hypothesis.
    # Find the largest hypothesis in either the shared driver or recipient that is NOT the hypothesis itself. Subtract by that value.
    if hypothesis.driverToken == hypothesis.recipientToken:
        hypothesis.hypothesis = 0.0 # dunno why this creeps in :/ KK
    if one_to_one:
        for hypothesis in memory.mappingHypotheses:
    ...
    # ADDED BY KK. To remove 1-1 mapping constraint
    else:
        for hypothesis in memory.mappingHypotheses:
            hypothesis.max_hyp = 0.0
...
def retrieval_routine(memory, asDORA, gamma, delta, HebbBias, lateral_input_level, bias_retrieval_analogs):
    ...
    if bias_retrieval_analogs:
        # for each analog, track the total activation of its units if they are in memory (i.e., if the analog is not
        # already in driver or recipient).
        for analog in memory.analogs:
...
def retrieve_tokens(memory, bias_retrieval_analogs, use_relative_act):    
    # if bias_retrieval_analogs is true, bias towards retrieving whole analogs. Otherwise, default to no bias
    # (myPs, RBs, and POs stand some odds of being retrieved regardless of their interconnectivity (of course, if a
    # token is retrieved, all tokens below it that the token is connected to are also retrieved)).
    if use_relative_act:
        ...
        if bias_retrieval_analogs:
            ...
            if len(analog_activation_list) > 0:
            ...
            for analog in memory.analogs:
                ...
                print analog.myPs[0].name, analog.total_act
                if analog.total_act > 0 and analog.num_units > 0:
                    retrieve_prob = analog.normalised_retrieval_act/sum_analog_norm_act
                    randomNum = random.random() # changed by KK - instead of random number, use a fixed threshold
                    #randomNum = 0.1 # need to play around with this (maybe later)
        ...
        if bias_retrieval_analogs:
            # retrieve whole analogs.
            # create a normalised retrieval score for each analog (i.e., analog.total_act/analog.num_units) and sum up
            # all normalised retrieval scores for each analog in memory.
...
def schematization_routine(memory, gamma, phase_set_iterator):
    # for each driver token unit, if that unit is most active unit of its type (e.g., most active P), and maps to a recipient 
    # unit, then check if you have caused a unit to be inferred in newSet. If you have caused a unit to be inferred, 
    # then set that newSet unit's activation to 1.0, and update connections to other newSet units (myPs to RBs, RBs to 
    # Ps and POs, POs to RBs), and for POs to semantic units. If not, then cause a new unit to be inferred.
    ...
    if most_active_P:
        ...
        if most_active_P.my_made_unit:
            ...
            if (most_active_P.act >= .4) and (most_active_P.max_map >= .75):
                # infer a newSet P unit (with activation 1.0) and add it to memory. Set the value of the .myanalog field 
                # to 'null', as you will create an analog to house all newSet units at the end of the .doSchematization() routine above. 
...
def rel_gen_routine(memory, gamma, recip_analog):
    # for relational generalization: Recall that only RBs in the driver from the analog that contains mapped tokens are 
    # firing. If active driver unit maps to no unit in the recipient, then infer a unit in recipient, set it's activation 
    # to 1, and attach it to active units above and below itself (e.g., connect new RBs to Ps and POs, new POs to RBs and 
    # semantics, etc.) to which it is NOT already connected. Make sure to mark that the new unit is inferred from the 
    # active driver unit (i.e., update .my_made_unit of driver unit and .my_maker_unit of new recipient unit). Newly inferred 
    # tokens become part of the same analog as the items in the recipient that map to the analog in the driver that contains 
    # the tokens that are currently driving relational generalisation. For example, if the tokens in the driver are part 
    # of analog 1, and analog 1 elements map to items in the recipient that are in analog 3, the newly inferred units in 
    # the recipient become part of analog 3.
...
def print_semantic_weight(obj):
    for sem in obj.mySemantics:
        print sem.mySemantic.name + ' : ' + str(sem.weight)

def print_semantic_act(obj):
    for sem in obj.mySemantics:
        print sem.mySemantic.name + ' : ' + str(sem.mySemantic.act)

def print_semantic_input(obj):
    for sem in obj.mySemantics:
        print sem.mySemantic.name + ' : ' + str(sem.mySemantic.myinput)


def print_inp(obj):
    print 'td, bu, lateral, map : ', str(obj.td_input),str(obj.bu_input),str(obj.lateral_input),str(obj.map_input)

def get_immediate_acts(unit):
    act_unit = []
    if unit.my_type == 'P':
        for unit_RBs in unit.myRBs:
            act_unit.append(unit_RBs.act)
    if unit.my_type == 'RB':
        act_unit.append(unit.myPred[0].act)
        if len(unit.myChildP) > 0:
            act_unit.append(unit.myChildP[0].act)
        else:
            act_unit.append(unit.myObj[0].act)
    return act_unit

def get_cosine_distance(l1,l2):
    if len(l1) != len(l2):
        return 0.0
    l1,l2 = np.array(l1),np.array(l2)
    return np.dot(l1,l2)

def get_RB_acts(rb_unit):
    act_rb_unit = []
    if len(rb_unit.myPred) > 0:
        act_rb_unit.append(rb_unit.myPred[0].act)
    if len(rb_unit.myObj) > 0: # Since only binary branching considered, only one predicate, and one object
        act_rb_unit.append(rb_unit.myObj[0].act)
    elif len(rb_unit.myChildP) > 0:
        for childP in rb_unit.myChildP:
            act_rb_unit.append(get_P_acts(childP))
    return act_rb_unit

def get_P_acts(p_unit):
    act_p_unit = []
    for rb_unit in p_unit.myRBs:
        act_p_unit.append(get_RB_acts(rb_unit))
    return act_p_unit


def get_activations_of_children(obj):
    return get_P_acts(obj)
...
def get_correlations(l1, l2):
    if len(l1) != len(l2):
        return 0.0
    l1,l2 = np.array(l1),np.array(l2)
    return np.corrcoef(l1,l2)[0,1]

def print_hyps(memory):
    for hypothesis in memory.mappingHypotheses:
        print hypothesis.driverToken.name, hypothesis.recipientToken.name, hypothesis.hypothesis

def print_RB_acts(memory):
    for unit_RB in memory.RBs:
        print unit_RB.name, unit_RB.act
def print_PO_acts(memory):
    for unit_PO in memory.POs:
        print unit_PO.name, unit_PO.act
def print_P_acts(memory):
    for unit_P in memory.Ps:
        print unit_P.name, unit_P.act
def print_sem_acts(memory):
    for unit_sem in memory.semantics:
        print unit_sem.name, unit_sem.act

def get_unit_level(unit):
    unit_level = 0
    if unit.my_type == 'P':
        list_units = []
        for unit_RBs in unit.myRBs:
            list_units.append(get_unit_level(unit_RBs))
        if len(unit.myRBs) > 1:
            unit_level = max(list_units) + 1
        else:
            unit_level = max(list_units)
    if unit.my_type == 'RB':
        if len(unit.myChildP) > 0:
            unit_level = 1 + get_unit_level(unit.myChildP[0])# add unit_level from lower levels
        else:
            unit_level = 2 # predicate + object
    if unit.my_type == 'PO':
        unit_level = 1
    return unit_level

def update_memory_with_levels(memory):
    for unit_Ps in memory.Ps:
        unit_Ps.unit_level = get_unit_level(unit_Ps)
    for unit_RBs in memory.RBs:
        unit_RBs.unit_level = get_unit_level(unit_RBs)
    for unit_POs in memory.POs:
        unit_POs.unit_level = get_unit_level(unit_POs)
    return memory

def get_POs(unit_P):
    all_POs = []
    for unit_RB in unit_P.myRBs:
        if len(unit_RB.myPred) > 0:
            all_POs.append(unit_RB.myPred[0]) # only one predicate possible
        if len(unit_RB.myObj) > 0:
            all_POs.append(unit_RB.myObj[0])
        elif len(unit_RB.myChildP) > 0:
            all_POs.append(get_POs(unit_RB.myChildP[0]))
    all_POs = flatten(all_POs)
    return all_POs

def init_store_activations(memory):
    # set all activation arrays to zeros of shape (num_units, 1)
    # return arrays of POs, RBs and Ps
    num_POs = len(memory.POs)
    num_RBs = len(memory.RBs)
    num_Ps = len(memory.Ps)
    return np.zeros((num_POs,)),np.zeros((num_RBs,)), np.zeros((num_Ps,))

def get_all_activations(memory,array_PO, array_RB, array_P):
    # add activations of all units in memory to corresponding unit in arrays
    list_PO,list_RB,list_P = [],[],[]
    for unit_PO in memory.POs:
        list_PO.append(unit_PO.act)
    for unit_RB in memory.RBs:
        list_RB.append(unit_RB.act)
    for unit_P in memory.Ps:
        list_P.append(unit_P.act)
    list_PO, list_RB, list_P = np.array(list_PO), np.array(list_RB), np.array(list_P)
    array_PO = array_PO + list_PO
    array_RB = array_RB + list_RB
    array_P = array_P + list_P
    return array_PO, array_RB, array_P