...
class PUnit(TokenUnit):
    def __init__(self, my_name, my_set, analog, inferred_now, myanalog):
        ...
        self.inhibitorThreshold = 40#100#440 ######### NOTE: THIS MIGHT NEED TO BE CHANGED LATER.
...
class RBUnit(TokenUnit):
    def __init__(self, my_name, my_set, analog, inferred_now, myanalog):
        ...
        self.timesFired = 0.0
        self.inhibitorThreshold = 20#220 ######### NOTE: THIS MIGHT NEED TO BE CHANGED LATER.
    ...
    def update_input_driver(self, memory, asDORA):
        ...
        ## HOW CAN YOU HAVE BOTH OBJECT AND CHILD Ps ?!
...
class POUnit(TokenUnit):
    def __init__(self, my_name, my_set, analog, inferred_now, myanalog, am_pred):
        ...
        self.inhibitorThreshold = 10#25#110 ######### NOTE: THIS MIGHT NEED TO BE CHANGED LATER.
...
class Semantic(object):
    ...
    def update_input(self, memory, ignore_object_semantics=False, ignore_memory_semantics=False):
        ...
        for Link in self.myPOs:
            # make sure that I'm not getting input from newSet POs, that I'm ignoring input from object POs if
            # ignore_object_semantics == True, and that I'm not getting input from memory units during retrieval if
                    ...
                    if Link.myPO.set != 'memory':
                        if ignore_object_semantics:
                            if Link.myPO.predOrObj==1:
                                ...
                                self.myinput += Link.myPO.act * Link.weight ## Don't ignore object Semantics (KK)
                        else:
                            self.myinput += Link.myPO.act * Link.weight
                else:
                    if ignore_object_semantics:
    ...
    def update_act(self): ## CHANGED BY KK
        self.act += self.myinput
        if self.act > 1.0:
            self.act = 1.0
        if self.act < 0.0:
            self.act = 0.0
...
class mappingHypothesis(object):
    def __init__(self, myDriverToken, myRecipientToken, mappingConnection):
        ...
        self.parent_bias = 0.0

    def update_hyp(self, memory, order_activation_bias=0.1,correlation_vals=0.0):
        ...
        if self.driverToken.my_type == 'P':
            ...
            if self.driverToken.mode == self.recipientToken.mode:
                if self.driverToken.unit_level == self.recipientToken.unit_level:
                    self.hypothesis = self.hypothesis + (self.driverToken.act * self.recipientToken.act)
                    self.hypothesis = self.hypothesis + (order_activation_bias * correlation_vals)
            ...
            if self.driverToken.predOrObj == self.recipientToken.predOrObj:
                self.hypothesis = self.hypothesis + (self.driverToken.act * self.recipientToken.act)
        elif self.driverToken.my_type == 'RB':
            if self.driverToken.unit_level == self.recipientToken.unit_level:
                self.hypothesis = self.hypothesis + (self.driverToken.act * self.recipientToken.act)
                self.hypothesis = self.hypothesis + (order_activation_bias * correlation_vals)
...
class globalInhibitor(object):
    ...
    def fire_global_inhibitor(self, memory, kk_style=False):
