...
def term_network_display(memory, set_to_display, display_sem=True):
...
def term_map_display(memory,only_Ps=False, only_max=True):
    ...
    if not only_Ps and not only_max:
        for group in memory.driver.Groups:
        ...
        for myPO in memory.driver.POs:
            ...
            print PO_string
    if only_Ps: ## ADDED BY KK
        if not only_max:
            for myP in memory.driver.Ps:
                for myMappingConnection in myP.mappingConnections:
                    if myMappingConnection.weight > 0:
                        P_string = 'P: ' + myP.name + ' -- ' + myMappingConnection.recipientToken.name + \
                                   ' -- ' + str(myMappingConnection.weight)
                        print P_string
                #else:
                #    P_string = 'P: ' + myP.name + ' -- ' + 'NONE' + ' -- mapping_weight=', myP.max_map
                #print
