...
# run_order is an array indicating the order of operations for a learning phase (cdr=clear driver and recipient, cr=clear
# recipient, selectTokens=select token units from memory to place in the driver (the selection of tokens is by analog
# (i.e., a specific analog is chosen, and all tokens from that analog are placed in the driver)), selectP=select a P at
# random from memory to place in the driver, , r=retrieval, m=map, p=predicate, f=form new relation, g=relational generalization,
# s=schema induction, 'b'=between group entropy ops, 'w'=within group entropy ops, 'wp'=within group entropy ops for preds only,
# (NOTE: b, w, and wp operations are newer to the theory and therefore listed after the old DORA operations; in reality according
# to the theory, they should occur after retrieval and before mapping) 'co'=compression (NOTE: compression is an old part
# of the theory covered in Doumas, 2005, but newly implemented for general DORA), c=clear results, cl=limited clear results
# (just inferences and newSet), wdr= write the current state of the driver and recipient to output file, wn=write current
# state of the network to output file).
# ignore_object_semantics is a parameter used when running in LISA mode. It allows DORA to downweight object semantics
# when multiple propositions are in the driver together. NOTE: I'm not sure if placing this item in the parameters dict
# is the best long-term solution. Likely, you'll want to integrate switching ignore_object_semantics automatically in
# the model based on context.
parameters = {'asDORA': False, 'gamma': 0.3, 'delta': 0.1, 'eta': 0.9,
              'HebbBias': 0.5,'bias_retrieval_analogs': True,
              'use_relative_act': True, 'run_order': ['cdr','selectTokens','r','m'],
              #'run_order': ['cdr','selectTokens','r','m'],
              #['cdr', 'selectTokens', 'r', 'wp', 'm', 'p', 'f', 's', 'c'],
              'run_cyles': 50, 'write_on_iteration': 100, 'firingOrderRule': 'by_top_random', 'strategic_mapping': False,
              'ignore_object_semantics': True, 'ignore_memory_semantics': True, 'mag_decimal_precision': 0,
              'dim_list': ['height', 'width', 'depth', 'size'], 'exemplar_memory': False, 'recent_analog_bias': True,
              'lateral_input_level': 1, 'screen_width': 1200, 'screen_height': 700, 'doGUI': True, 'GUI_update_rate': 50,
...
class MainMenu(object):
    def __init__(self):
        ...
        self.file = open('karthikeya/test_sim_6.py', 'r')
            ...
            if run_type.upper() == 'D':
                ...
                while go_on_run:
                    ...
                    if run_command.upper() == 'V':
                        ...
                        # do map.
                        self.network.do_map()
                        basicRunDORA.DORA_GUI.term_map_display(self.network.memory)
